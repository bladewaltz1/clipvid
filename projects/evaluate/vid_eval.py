...
def do_vid_evaluation(dataset, predictions, output_folder, motion_specific, logger, box_only=False):
    pred_boxlists = []
    gt_boxlists = []
    for image_id, prediction in enumerate(predictions):
        ...
        prediction_ = BoxList(prediction["bbox"], (image_width, image_height))
        prediction_.add_field("scores", prediction["scores"])
        prediction_.add_field("labels", prediction["labels"])
        pred_boxlists.append(prediction_)
        ##############
...
def eval_detection_vid(pred_boxlists,
    ...
    if motion_specific:
        motion_iou_file = "vid_groundtruth_motion_iou.mat"
...
def calc_detection_vid_prec_rec(gt_boxlists, pred_boxlists, motion_ious, iou_thresh=0.5, motion_range=[0., 1.]):
    ...
    for gt_boxlist, pred_boxlist, motion_iou in zip(gt_boxlists, pred_boxlists, motion_ious):
        ...
        ########
        pred_label = pred_boxlist.get_field("labels").numpy() + 1
        ########
